...
public class mainintegral {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                for (int k = 0; k < geo.numProton.length; k++) {
                    ak1++;
                    while (Thread.activeCount() > Integer.parseInt(kernel.a.thr_nri.getText())) {
                    }
    ...
    public void two(String name) throws InterruptedException {
        kernel.a.proses_int.setMinimum(0);
        kernel.a.proses_int.setMaximum(100);
        kernel.a.proses_int.setValue(0);
        double iteration = 0;
            ...
            for (int j = 0; j < n; j++) {
                ...
                for (int k = 0; k < n; k++) {
                    for (int l = 0; l < n; l++) {
                        if (cek[i][j][k][l] == 0) {
                            ak++;
                            while (Thread.activeCount() > Integer.parseInt(kernel.a.thr_eri.getText())) {
                            }
